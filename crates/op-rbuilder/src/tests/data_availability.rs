...
/// We will set limit to 1 byte and see that the builder will not include any
/// transactions.
#[rb_test]
async fn tx_size_limit(rbuilder: LocalInstance) -> eyre::Result<()> {
    let driver = rbuilder.driver().await?;

    // Set (max_tx_da_size, max_block_da_size), with this case block won't fit any
    // transaction
    let call = driver
        .provider()
...
/// We will set limit to 3 txs and see that the builder will include 3
/// transactions. We should not forget about builder transaction so we will
/// spawn only 2 regular txs.
#[rb_test]
async fn block_fill(rbuilder: LocalInstance) -> eyre::Result<()> {
    ...
    // We already have 2 so we will spawn one more to check that it won't be
    // included (it won't fit because of builder tx)
    ...
    assert!(!block.includes(unfit_tx_3.tx_hash()), "unfit tx should not be in block");
    assert!(
